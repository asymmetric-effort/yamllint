export function cliPage(): string {
  ...
  const optionsRows = options
    .map(([flag, desc]) => `<tr><td>${flag}</td><td>${desc}</td></tr>`)
    .join("");
  ...
  return `
    ...
    <div class="section">
      ...
      <h3>Lint a directory recursively</h3>
      <pre><code>yamllint ./ansible/</code></pre>
...
rules:
  ...
  document-start: disable" .</code></pre>
      ...
      <pre><code>cat deployment.yaml | yamllint -</code></pre>
    </div>
    ...
    <div class="section">
      ...
      <table>
        ...
        <tbody>
          ...
          <tr><td>2</td><td>&mdash;</td><td>Warnings only</td></tr>
        </tbody>
      </table>
    </div>
  `;
